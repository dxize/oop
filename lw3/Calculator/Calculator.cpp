...
Variable& Calc::GetVar(const std::string& name)
{
	...
	{
		static Variable dummy; // та сама€ "пуста€" переменна€ √ЋяЌ”“№ „“ќ Ё“ќ!!!!
		return dummy;
	}
}
...
void Calc::SetLet(const std::string& name, const std::string& value)
{
	...
	if (pos == value.size()) // ≈сли все символы строки обработаны, то pos должен равн€тьс€ размеру строки(если число)
	{
